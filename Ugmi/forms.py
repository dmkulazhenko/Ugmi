...
class ContactUsForm(FlaskForm):
    ...
    def validate(self):
        ...
        ac = True
        if re.match("^[A-Za-zА-Яа-я ]*$", self.name.data) is None:
            self.name.errors.append(u'Не похоже на Ваше имя.')
            ac = False
        if re.match("^[0-9 +#()-]*$", self.phone.data) is None:
        ...
        for junk_char in '#()- ':
            self.phone.data = self.phone.data.replace(junk_char, '')
...
class RegistrationForm(FlaskForm):
    ...
    password_conf = PasswordField('password_conf', [EqualTo('password', message='Пароли не совпадают.')],
                                  render_kw={"placeholder": u"* Пароль"})
    ...
    def validate(self):
        ...
        if re.match("^[A-Za-zА-Яа-я ]*$", self.name.data) is None:
            self.name.errors.append(u'Не похоже на Ваше имя.')
            ac = False
        if re.match("^[A-Za-z0-9_]*$", self.username.data) is None:
...
class LoginForm(FlaskForm):
    ...
    def validate(self):
        ...
        if (user is None) or (not user.auth(self.password.data)):
            self.login.errors.append(u'Неверный логин или пароль.')
            return False

        if not user.confirmed:
            self.login.errors.append(Markup(
                u'Подтвердите ваш email. Для отправки повторного письма тыкните <a href="'
                + url_for('resend_confirmation', username=user.username) + u'">сюда</a>.'
            ))
            return False
...
class PasswordResetForm(FlaskForm):
    ...
    def validate(self):
        ...
        if (user.last_password_reset is not None) and ((datetime.utcnow() - user.last_password_reset).total_seconds() < 900):
...
class PasswordResetSetForm(FlaskForm):
    ...
    password_conf = PasswordField('password_conf', [EqualTo('password', message='Пароли не совпадают.')],
                                  render_kw={"placeholder": u"Ещё разок"})
...
class AddSmallMarkForm(FlaskForm):
...
class GenerateSmallMarkForm(FlaskForm):
    ...
    def validate(self):
        ...
        if re.match("^[0-9]*$", self.mark_id.data) is None:
...
class EditSmallMarkForm(FlaskForm):
    title = StringField('title', [
        DataRequired(message=u'Введите заголовок.'),
        Length(min=3, max=256, message=u'Сликшом короткий или длинный заголовок.)')
    ], render_kw={"placeholder": u"*Заголовок"})
